...
class PoissonDisc():
    ...
    def __init__(self, width=50, height=50, r=1, k=30, n=10, invalid_points=set([]), seed=10):
        np.random.seed(seed)

        self.width, self.height = width, height
        self.r = r #minimum distance between two points
        self.k = k #num of candidates to sample in an interation
        self.n = n #num of points to sample
        self.invalid_points = invalid_points #points which should not be sampled.
    ...
    def point_valid(self, pt):
        ...
        if (np.floor(pt[1]), np.floor(pt[0])) in self.invalid_points:
            return False
    ...
    def sample(self):
        ...
        while 1:
            pt = (np.floor(np.random.uniform(0, self.width)), np.floor(np.random.uniform(0, self.height)))
            if self.point_valid(pt):
                break
        ...
        # samples.
        while active and len(self.samples) < self.n:
            ...
            if pt:
                ...
                self.samples.append((np.floor(pt[0]), np.floor(pt[1])))
